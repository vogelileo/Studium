...
valid Leaf = undefined
valid (Node ltt x gtt) = undefined  -- Hint: Read `ltt` as "less than tree" and `gtt` as "greater than tree"
...
insert e Leaf = undefined
insert e (Node ltt p gtt) = undefined
...
toList :: T a -> [a]
toList Leaf = undefined
toList (Node l x r) = undefined


member :: Ord a => a -> T a -> Bool
member x Leaf = undefined
member x (Node l y r) = undefined
...
arbitraryLimited = sized tree
  where
    tree :: Int -> Gen (T Int)
    tree 0 = pure empty
    tree n  | n > 0 = insert <$> choose (1, 4) <*> oneof [pure empty, tree (n-1)]
